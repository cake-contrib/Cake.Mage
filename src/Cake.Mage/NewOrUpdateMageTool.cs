...
internal class NewOrUpdateMageTool : MageTool<BaseNewAndUpdateMageSettings>
{
    ...
    private ProcessArgumentBuilder GetNewOrUpdateArguments(BaseNewAndUpdateMageSettings settings)
    {
        ...
        else
        {
            ...
            {
                builder = builder.Append("Mage -New Deployment");
            }
            else
            {
        ...
        return builder
            .AppendIfNotDefaultSwitch("-a", settings.Algorithm, Algorithm.SHA256RSA)
    ...
    internal NewOrUpdateMageTool(IFileSystem fileSystem, ICakeEnvironment environment, IProcessRunner processRunner, IToolLocator tools, IRegistry registry, ICakeLog log) : base(fileSystem, environment, processRunner, tools, registry, log)
    {
    }
}